...
def learn_vector_relationship(dict1, dict2, key_list, epochs=1024, batch_size=32, validation_split=0.2):
    ...
    plt.title('Model Loss Over Epochs learning glove 300 from w2v')
    plt.xlabel('Epoch')
    plt.ylabel('Loss (MSE)')
    plt.legend()
    plt.savefig("learning_loss_from_w2v_to_big_glove.png")
    plt.show()
...
if __name__ == "__main__":
    ...
    model = learn_vector_relationship(w2v_vectors, glove_vectors, wordlist)

    # Prepare the input matrix for all words in the wordlist
    input_vectors = np.array([w2v_vectors[word.lower()] for word in wordlist])
    ...
    model.save('w2v_to_glove300_model.keras')
